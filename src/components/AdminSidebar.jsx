...
const ROUTING_NAV = [
    { id: 'routing-intel', label: 'Intelligence', icon: 'brain' },
    { id: 'api-content', label: 'API Requests', icon: 'database' },
];
...
const ICONS = {
    ...
    brain: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 00-2.456 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423L16.5 15.75l.394 1.183a2.25 2.25 0 001.423 1.423l1.183.394-1.183.394a2.25 2.25 0 00-1.423 1.423z" /></svg>,
    database: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 5.625c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125" /></svg>,
};
    ...
    return (
        <aside className={`admin-sidebar flex flex-col transition-all duration-200 ${collapsed ? 'w-16' : 'w-56'}`}
            ...
            <nav className="flex-1 py-2">
                ...
                {/* Divider with routing intelligence label */}
                <div className="mx-3 my-3">
                    <div className="h-px bg-[var(--border-secondary)]" />
                </div>
                {!collapsed && (
                    <div className="px-4 pb-1 text-[10px] font-medium text-[var(--text-tertiary)] uppercase tracking-wider" style={{ color: '#34D399' }}>
                        Routing Intelligence
                    </div>
                )}
                {collapsed && <div className="h-px bg-[var(--border-primary)] mx-2 my-1" />}
                {ROUTING_NAV.map((item) => (
                    <NavItem key={item.id} item={item} isActive={activeTab === item.id}
                        onClick={() => onTabChange(item.id)} collapsed={collapsed} isAgent={false} />
                ))}
