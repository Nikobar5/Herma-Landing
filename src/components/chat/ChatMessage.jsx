...
const ChatMessage = ({ message, isLast, isStreaming, onRegenerate }) => {
  ...
  return (
    <div className={`flex w-full px-4 md:px-6 py-6 transition-colors ${isAssistant ? 'bg-[var(--bg-secondary)]/50 backdrop-blur-sm border-y border-[var(--border-primary)]' : ''
      }`}>
      <div className="w-full max-w-4xl mx-auto flex gap-4 md:gap-6">
        {/* Avatar */}
        <div className="flex-shrink-0">
          {isAssistant ? (
            <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold bg-[var(--accent-primary)] text-white shadow-lg shadow-[var(--accent-primary)]/20" style={{ fontFamily: 'var(--font-heading)' }}>
              H
            </div>
          ) : (
            <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold bg-[var(--bg-tertiary)] text-[var(--text-secondary)] border border-[var(--border-secondary)] shadow-sm" style={{ fontFamily: 'var(--font-heading)' }}>
              U
            </div>
          )}
        </div>
